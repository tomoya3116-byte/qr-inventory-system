...
| `QR_INVENTORY_DB_PATH` | 推奨 | `data/inventory.db` | SQLite DBファイルの保存先です。相対パスはリポジトリルート基準で解釈されます。永続化されるディスク上のパスを指定してください。 |
...
SQLite 運用では `QR_INVENTORY_DB_PATH` で指定した DBファイルが最重要データです。既定のDB、バックアップ、QR画像、ラベルHTMLはリポジトリルート基準の `data/inventory.db`、`backups/`、`qr_codes/`、`labels/` に保存されるため、起動したカレントディレクトリに不要な生成物が散らばりません。最低限、次の方針でバックアップしてください。
...
- DBファイル、`backups/`、`qr_codes/`、`labels/` など、運用に必要な生成物の保存場所を把握してください。
