...
- CSV取込: 管理者ログイン後、`/csv-import` でCSVファイル選択フォームを表示できます。現時点では、Web画面からのCSV取込実行・プレビュー確定フローは整備中です。DB側にはCSV読込、文字コード判定、ヘッダー検証、登録/更新判定、エラー検出の処理を用意していますが、利用者向け画面では限定実装として扱います。
