...
Ver2.0 Web Phase 3 では、既存のCUI版・GUI版を残したまま、FastAPIベースのWeb版を `src/web_app.py` として拡張しています。
スマートフォンやPCブラウザから、トップページ、品目一覧、品目検索、入庫、出庫、最低在庫アラート、品目登録・編集・削除、棚卸修正に加えて、CSV取込、QRコード生成、ラベル印刷用HTML生成、DBバックアップ、DB復旧を利用できます。
...
### Web版 Phase 3 で利用できる機能

- 品目登録: `/items/new` から品目ID、品名、最低在庫数、初期在庫数などを入力して登録できます。
- 品目編集: 品目一覧のカードから編集画面へ移動し、品目マスタの基本情報を更新できます。
- 品目削除: 編集画面から削除確認へ進み、確認用の品目ID入力後に削除できます。
- 棚卸修正: `/stock-adjust` から実在庫数を入力し、ADJUST履歴として差異を記録できます。
- 管理者メニュー: `/admin` から品目登録、品目編集・削除、棚卸修正、最低在庫確認、CSV取込、QRコード、ラベル印刷、DBバックアップ、DB復旧へ移動できます。
...
- DB復旧: `/db-restore` でバックアップ一覧から復旧元を選択します。復旧前に現在DBを `backups/auto_before_restore_YYYYMMDD_HHMMSS.db` として自動バックアップし、誤操作防止のため確認文字列 `RESTORE` の入力を要求します。
