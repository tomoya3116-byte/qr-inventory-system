...
管理者ログイン、品目マスタ管理、入出庫、棚卸修正、CSV出力、QRコード生成、ラベル印刷、DBバックアップ・復旧、操作ログ、検索・絞り込み・並び替え、`/scan/{item_id}` 形式のQRスキャン運用を追加済みです。

CSV取込は画面とDB側の取込処理を整備中です。実運用向けの取込実行フローは、今後の更新で整理します。

## 主な機能

- **Web版**: スマートフォンやPCブラウザから、品目一覧、品目検索、入庫、出庫、最低在庫アラート、管理者機能を利用できます。
- **実用機能**: CSV出力、QRコード生成、ラベル印刷用HTML生成、DBバックアップ、DB復旧、操作ログ、`/scan/{item_id}` 形式のQRスキャン運用に対応しています。
- **整備中の機能**: CSV取込は、画面・DB処理・プレビュー確認を含めた実運用向けフローを整備中です。
- **運用機能**: 環境変数によるDB保存先、管理者パスワード、セッション署名キーの設定に対応しています。
