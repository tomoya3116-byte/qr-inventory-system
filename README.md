...
Ver2.0 Web Phase 2 では、既存のCUI版・GUI版を残したまま、FastAPIベースのWeb版を `src/web_app.py` として拡張しています。
スマートフォンやPCブラウザから、トップページ、品目一覧、品目検索、入庫、出庫、最低在庫アラートに加えて、品目登録・編集・削除と棚卸修正を利用できます。
...
- 管理者メニュー: `/admin` から品目登録、品目編集・削除、棚卸修正、最低在庫確認へ移動できます。
